...
(() =>{
    ...
    getRequest(`${SW_API}/people/2`).pipe(
        ...
        // Primer prueba - ejercicio 1
        // switchMap(data => ajax.getJSON(data['species'][0]))


        // Zip permite empaquetar o combinar el resultado de varios Observables como un arreglo de valores.
        switchMap(data => zip(of(data), ajax.getJSON(data['species'][0]))),
        // Transformar la salida en un objeto con la respuesta del peronsaje y su respectiva especie
        map(([peronaje, especie]) => ({peronaje, especie}) )
